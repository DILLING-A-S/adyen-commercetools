...
| `commercetools` | `authentication`             | HTTP Basic Auth credentials (`{ "scheme": "basic", "username": "...", "password": "..." }`) used to authenticate notifications from webhooks that Adyen does **not** HMAC-sign, e.g. the [Generic Pending webhook](https://docs.adyen.com/development-resources/webhooks/webhook-types) (`eventCode: PENDING`). Configure the same credentials as Basic Auth on that webhook in the Adyen Customer Area. Only required if you receive such webhooks. |                                                                                                       |
